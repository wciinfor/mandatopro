...
function detectFollowUpIntent(message) {
  const normalized = normalizeForMatch(message);
  const followUpRegex = /(\beles\b|\bdeles\b|\bessas\b|\besses\b|\bquais\b|\bquais sao\b|\bliste\b|\blistar\b|\bmostre\b|\btodos\b|\btodas\b|\btods\b|\bos contatos\b|\bos anteriores\b|\bdaqueles\b|\bo primeiro\b|\bo segundo\b|\bo terceiro\b|\ba primeira\b|\ba segunda\b|\ba terceira\b|\bprimeiro\b|\bsegundo\b|\bterceiro\b)/;
  ...
  return {
    isFollowUp: followUpRegex.test(normalized) || wantsContacts || wantsLocation || wantsList || wantsContinue || wantsAll || wantsPriority || confirmsPrevious || ordinalIndex !== null,
    wantsContacts,
    wantsLocation,
    wantsList,
    wantsContinue,
...
function resolveFollowup(question, lastContext) {
  ...
  } else if (followUp.wantsList || followUp.wantsAll || followUp.wantsContinue || (followUp.confirmsPrevious && lastContext?.lastPlan?.action === 'count')) {
    intent = 'list';
  }
  ...
  return {
    ...
    reuseLastFilters: (followUp.wantsList || followUp.wantsAll || followUp.wantsContinue || followUp.wantsPriority || followUp.confirmsPrevious || ids.length === 0) && (!targetIds || targetIds.length === 0)
  };
}
...
function countLabelForTable(table) {
  const phrase = countPhraseForTable(table);
  const noun = phrase.noun || 'registros';
  const singularMap = {
    campanhas: 'campanha',
  ...
  return {
    plural: noun,
    singular: singularMap[table] || noun.replace(/s$/, ''),
    suffix: phrase.suffix || 'cadastrados'
  };
}
...
function buildAnswerLocal(plan, rows, meta, question) {
  ...
  if (plan?.action === 'count') {
    ...
    if (total === 0) {
      return `Nao encontrei ${label.plural} ${label.suffix}${scopeText}. Se quiser, posso verificar por outro termo, cidade, bairro ou status.`;
    }
    const noun = total === 1 ? label.singular : label.plural;
    return `Temos ${total} ${noun} ${label.suffix}${scopeText}. Quer que eu liste quais sao?`;
  }
...
async function buildAnswer(plan, rows, meta, question, provider, historyMessages, conversationContextBlock) {
  ...
  const limit = Math.min(allSnippets.length || 0, plan?.limit || LIST_PAGE_SIZE, LIST_PAGE_SIZE);
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (followUpActive && resolvedPlan.action !== 'list' && !resolvedPlan.filters?.ids && !resolvedPlan.search && !hasFollowUpFilter) {
    ...
    if (rows.length > 0 && conversationKey) {
      const newContext = withContextLocation(
        buildLastContext(traceId, resolvedPlan, rows, totalRows ?? rows.length),
        lastContext,
        resolvedPlan
      );
      setConversationContext(conversationKey, newContext);
