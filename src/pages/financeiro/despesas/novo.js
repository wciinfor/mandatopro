...
export default function NovaDespesa() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.data_despesa || !formData.tipo || !formData.valor) {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data?.message || 'Erro ao criar despesa');
      }
    ...
    <Layout titulo="Nova Despesa">
      ...
      <div className="bg-white rounded-lg shadow-sm p-6">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-lg font-semibold text-gray-800">Cadastro de Despesa</h3>
        </div>
